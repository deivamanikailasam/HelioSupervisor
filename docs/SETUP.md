...
**RAG (local documents):** For the `rag_search` tool, the stack includes `faiss-cpu`, `sentence-transformers`, `pypdf`, `pymupdf` (PDF fallback), and `langchain-text-splitters`. Default embedding model is `BAAI/bge-base-en-v1.5` (first run downloads it). In the **Web UI**, RAG is opt-in per run: before sending a message, attach a document (saved to `memory/docs/`), and/or select documents or folders from the “RAG for this run” expander. If you do none of these, RAG is not used for that run. When scope is set, a scoped in-memory index is built from the selected paths. You can also add `.md`, `.txt`, or `.pdf` files directly under `memory/docs/` (including subfolders) and select them in the UI. Use **"Answer only from documents (offline)"** in the expander to restrict answers to the selected docs (no web fetch). PDF text is extracted with pypdf first, then PyMuPDF when needed. If RAG dependencies are missing, the tool falls back to simple keyword matching.
