...
| **rag.py** | Minimal RAG: loads Markdown/PDF/text from `memory/docs/` (PDF: pypdf then PyMuPDF), chunks with `RecursiveCharacterTextSplitter`, indexes with FAISS + BGE/default embeddings. **RAG opt-in per run:** UI passes `rag_scope` and `rag_documents_only`; when `rag_scope` is set, scoped in-memory index is built; fallback to first chunks when similarity search returns nothing. Exposes `list_docs_and_folders()`, `save_uploaded_to_docs()`, `expand_rag_scope()`, `set_rag_scope()` / `get_rag_search_fn()`. |
| **supervisor.py** | Builds system prompt (RAG anti-hallucination and documents-only when set) and tool list (omits web_fetch when RAG documents-only); creates the agent graph with `create_agent()`; implements `run_supervisor()` (history, invoke, memory append, optional self-critique, gc.collect after RAG). Accepts `rag_scope` and `rag_documents_only`; sets RAG context so `rag_search` runs only over those docs or returns “no documents selected” message. |
| **ui.py** | Streamlit app: sidebar (provider, model, API keys, approval/critique); **RAG for this run** expander with "Answer only from documents (offline)" checkbox, file upload, multiselect for docs/folders; chat history (capped 40 messages), "Tools used" per reply. |
...
2. **UI or CLI** calls `run_supervisor(user_input, chat_history=..., api_keys=..., include_critique=..., require_human_approval=..., rag_scope=..., rag_documents_only=...)`. The UI builds `rag_scope` from attached files (saved to `memory/docs/`), selected documents, and expanded selected folders, and passes the "Answer only from documents (offline)" checkbox as `rag_documents_only`; if no docs are set, `rag_scope` is `None` and RAG is not used for that run.
...
- A **system prompt** (from `get_supervisor_system_prompt(require_human_approval, rag_scope, rag_documents_only)`): describes the role, tool use, approval behaviour, and when RAG is used adds anti-hallucination instructions and (if `rag_documents_only`) documents-only and no web_fetch.
- **Tools** from `get_supervisor_tools(include_web_fetch)`: when RAG documents-only is on, `web_fetch` is omitted; otherwise `plan_tasks_tool`, `web_fetch_tool`, `code_exec_tool`, `write_note_tool`, `summarize_text_tool`, `rag_search_tool`.
